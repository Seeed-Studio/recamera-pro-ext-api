...
Inference RV1126B 0.3.1 使用此接口。固件需包含 `market/appmgr/workflow_ui.py`、
...
记录没有图像输出时卡片仅展示 JSON，不以设备摄像头视频替代。此版本不向 Result Hub
发布归一化框，也不声明设备 OSD 能力，配置中不显示对此应用无效的实时视频叠加开关。
