...
- `src/` — C ABI 的完整客户端实现(结果/帧/probe/NPU lease/遮罩)
- `proto/` + `generated/` — 固定 wire schema 与 protobuf-c 1.4.1 生成结果
...
需要 CMake、Python 3、`readelf`、protobuf-c 开发文件，以及严格固定的
`protoc-c 1.4.1` / `libprotoc 3.21.12`。在完整 RV1126B SDK 工作区内脚本会
自动找到 Buildroot host 工具；独立 checkout 可用环境变量显式指定：

```sh
# x86_64 host build；系统已安装 libprotobuf-c-dev 时无需 PROTOBUF_C_PREFIX
PROTOC_C=/path/to/protoc-c \
PROTOBUF_C_PREFIX=/path/to/host/prefix \
scripts/test-native-sdk.sh --host

# RV1126B aarch64 cross build
PROTOC_C=/path/to/protoc-c \
AARCH64_CC=/path/to/aarch64-rockchip1240-linux-gnu-gcc \
AARCH64_SYSROOT=/path/to/arm64/sysroot \
scripts/test-native-sdk.sh --aarch64
```

门禁在临时目录中只复制 `sdk/` 后构建，随后检查 pinned generated bytes、
LP64 公共结构布局、冻结 C 符号、`librecamera_ext.so.1` SONAME、
protobuf-c ABI 依赖及构建目录软链。
...
## NPU 单 owner lease（native SDK v1.3.0+）

`rc_ext_inference_lease_*` / Python `InferenceLease` 连接
`/run/recamera/inference-control.sock`。连接本身就是租约；rkipc 先停妥并销毁
内建 RKNN context，确认 `handle == NULL`、`state == stopped`、
`actual_fps == 0` 后才返回 GRANT。初始化外部 RKNN 成功后调用 `ready()`；进程
崩溃或 `SIGKILL` 时 socket HUP 使服务端自动回收并按 fallback 恢复内建模型。

lease 带 `lease_id`/`epoch`/`generation`，且 fd 为 CLOEXEC。fork child 必须走
`rc_ext_inference_lease_abandon_after_fork()`，只关闭自己的继承副本，不能向
服务端发送 RELEASE。该版本只新增符号，SONAME 仍为 `.so.1`；
frame/result/probe 数据端点绑定 generation 留给下一协议版本。

## 部署到设备
- 当前固件源码构建会把 Python 包装入 `/usr/lib/python3.11/site-packages`，native
  library 装入 `/usr/lib`；历史 `/userdata/sdk` 手工布局才需要 PYTHONPATH/
  LD_LIBRARY_PATH。
