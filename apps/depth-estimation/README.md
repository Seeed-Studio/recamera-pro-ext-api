...
`apps/face-recognition/evaluation/depth-model-selection.md`（原始材料保留在
[`55a852d`](https://github.com/Seeed-Studio/recamera-pro-ext-api/tree/55a852d/apps/face-recognition/evaluation/depth-model-selection.md) 分支）。
...
uv run pytest apps/depth-estimation/tests -q

# 先按应用中心发布指南构建并安装签名 v2 包，再通过 appmgr 管理
python3 -m appmgr start depth-estimation
sh /userdata/local/apps/depth-estimation/restart.sh
sh /userdata/local/apps/depth-estimation/status.sh        # appmgr 应用状态
