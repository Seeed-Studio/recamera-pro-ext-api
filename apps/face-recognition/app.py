...
Enrollment is an HTTP command channel on 127.0.0.1:<cmd_port> (default 8126;
8125 belongs to the result hub; see cmd_server.py). Both sources run INSIDE
this loop, never on the HTTP thread:
...
class FaceRecognitionApp(App):
    ...
    def setup(self, config) -> None:
        ...
        if self._capture_dir is None:
            self._capture_dir = c.get("capture_dir") or os.path.join(
                kit_cfg.appdata_root(), self.id, "captures")
        self.cmd_port = int(c.get("cmd_port", self.cmd_port))
        if self.cmd_port == 8125:
            raise ValueError(
                "cmd_port 8125 is reserved by the result hub; use 8126 "
                "for face-recognition or 0 to disable the command endpoint"
            )
