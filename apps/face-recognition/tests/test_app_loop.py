...
class TestLoopContract(_Base):
    def test_default_cmd_port_does_not_take_result_hub_port(self):
        spec = next(item for group in self.manifest["config_schema"]["groups"]
                    for item in group["items"] if item["key"] == "cmd_port")
        assert spec["default"] == 8126
        assert spec["default"] != 8125
